...
from run_theory_vs_simulation import (
    build_safety_fee_vector_from_candidate,
    ...
    for t in range(cfg.max_rounds + 1):
        ...
        if safety_active:
            next_state, batch_info = apply_batch_safety_threshold_update(
                ...
                safety_fee_vector,
                c_star,
                dynamic_safety_fee=dynamic_safety_fee,
...
def build_run_safety_objects(
    ...
    rr_result = run_single_intervention_timing(
        cfg,
        params,
        seed_share,
        beta_star,
        safety_fee_vector=np.zeros(cfg.n_agents),
        c_star=float("inf"),
        initial_state=shared_initial_state,
        update_schedule=shared_update_schedule,
        intervention_round=None,
    )
    final_state = rr_result["final_state"]
    ...
    d_min_vector = build_safety_fee_vector_from_candidate(
        cfg,
        params,
        candidate_state,
        c_star=c_star,
...
def run_one_replication(
    ...
    for intervention_round in intervention_rounds:
        result = run_single_intervention_timing(
            ...
            seed_share_f=seed_share,
            beta=beta_star,
            safety_fee_vector=d_vector,
            c_star=c_star,
            initial_state=shared_initial_state,
            update_schedule=shared_update_schedule,
            intervention_round=intervention_round,
            allow_early_stop=allow_early_stop,
            dynamic_safety_fee=True,
            safety_fee_multiple=fee_multiple,
        )
...
def write_outputs(
    ...
    for idx, label in enumerate(ordered_labels):
        ...
        display = "RR only" if label == "never" else f"Safety from {label}"
    ...
    summary["plot_label"] = summary["intervention_label"].map(
        lambda label: "RR only" if label == "never" else label
    )
    ...
    with (output_dir / "README.md").open("w", encoding="utf-8") as f:
        ...
        f.write("The experiment varies only the round at which the safety-threshold layer is activated on top of RR.\n\n")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Comma-separated list of intervention rounds; use `never` for the RR-only baseline.",
    )
    parser.add_argument(
        "--fee-multiple",
        type=float,
