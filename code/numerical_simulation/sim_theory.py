...
def compute_rhood_theory(state: np.ndarray, params: pd.DataFrame, cfg: ExperimentConfig) -> Dict[str, object]:
    ...
    share_basis = rhood_share_basis_vector(state, params, cfg)
...
def scan_rhood_feasibility(
    state: np.ndarray,
    params: pd.DataFrame,
    cfg: ExperimentConfig,
) -> pd.DataFrame:
    theory = compute_rhood_theory(state, params, cfg)
    ...
    for beta in cfg.beta_grid:
        ...
        for idx in core_idx:
            ...
            u_rhood = base_share + compensation + k - float(realized_cost[idx])
            core_utilities.append(u_rhood)
...
def compute_deposit_theory(
    ...
    utility_block = compute_utilities_with_rhood(candidate_state, params, cfg, beta=beta)
    ...
    for idx in range(len(params)):
        ...
        if pre_action != STATE_F or cf_c_ext <= c_star or current_c_ext > c_star:
            ...
            noise = rng.normal(0.0, cfg.deposit_noise_sigma)
            pivot = float(np.clip(1.0 - noise, 0.05, 1.0))

        rhood_full_sharing_gain = float(
            utility_block["U_F_rhood"][idx]
            - max(float(utility_block["U_E_rhood"][idx]), float(utility_block["U_N_rhood"][idx]))
        )
        d_min = np.nan if pivot <= 0.0 else max(rhood_full_sharing_gain / pivot, 0.0)
        ...
        rows.append(
            {
                ...
                "rhood_upgrade_gain": rhood_full_sharing_gain,
                "D_min": d_min,
            }
        )
    return pd.DataFrame(rows)
...
def scan_deposit_deterrence(
    ...
    base_utils = compute_utilities_with_rhood(candidate_state, params, cfg, beta=beta)
    current_c_ext = compute_externality_total(candidate_state, params, cfg)
    ...
    for idx in range(len(params)):
        pre_action = choose_action(
            current=int(candidate_state[idx]),
            u_n=float(base_utils["U_N_rhood"][idx]),
            u_i=float(base_utils["U_E_rhood"][idx]),
            u_f=float(base_utils["U_F_rhood"][idx]),
        )
    ...
    for idx in range(len(params)):
        ...
        gain_from_deviation = float(
            base_utils["U_F_rhood"][idx]
            - max(float(base_utils["U_E_rhood"][idx]), float(base_utils["U_N_rhood"][idx]))
        )
        ...
        agent_rows.append(
            {
                ...
                "rhood_upgrade_gain": gain_from_deviation,
                "D_min": d_min,
            }
        )
    ...
    if not valid_agents.empty:
        for mult in cfg.deposit_multipliers:
            deterrence_flags = []
            for row in valid_agents.itertuples(index=False):
                deposit = mult * float(row.D_min)
                net_gain = float(row.gain_from_deviation) - float(row.pivot_probability) * deposit
                deterrence_flags.append(net_gain <= 1e-10)
            scan_rows.append(
                {
                    "deposit_multiple": float(mult),
...
def run_theory_closure_with_deposit(
    cfg: ExperimentConfig,
    params: pd.DataFrame,
    seed_share_f: float,
    beta: float,
...
def run_async_theory_with_deposit(
    cfg: ExperimentConfig,
    params: pd.DataFrame,
    beta: float,
    deposit_vector: np.ndarray,
...
def run_theory_closure_with_rhood(
    cfg: ExperimentConfig,
    params: pd.DataFrame,
    seed_share_f: float,
    beta: float,
...
def run_async_theory_with_rhood(
    cfg: ExperimentConfig,
    params: pd.DataFrame,
    beta: float,
    initial_state: np.ndarray,
