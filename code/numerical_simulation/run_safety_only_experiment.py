...
def build_primitive_safety_vector(
    ...
    for idx in batch_f_agents:
        ...
        safety_fee_vector[idx] = max(gain, 0.0)
    return safety_fee_vector
        ...
        if trigger:
            for idx in update_agents:
                ...
                revised_action = choose_action(
                    ...
                    u_n=float(utility_block["U_N"][idx]),
                    u_i=float(utility_block["U_E"][idx]),
                    u_f=float(utility_block["U_F"][idx] - active_safety_fee[idx]),
                )
        ...
        local_row.update(
            {
                ...
                "batch_mean_active_safety_fee": float(np.mean(active_safety_fee[update_agents]))
...
def run_experiment(
    ...
    for run_id in range(cfg.n_runs):
        ...
        safety_only = run_single_simulation_safety_only(
            ...
            params,
            seed_share_f=seed_share,
            safety_fee_vector=d_min_vector,
            c_star=c_star,
...
def write_outputs(
    ...
    if main_results_dir is not None:
        ...
        if main_welfare_path.exists():
            ...
            append_main_path("simulation_rr", "rr", "RR")
            append_main_path("simulation_safety_fee", "safety_fee", "DM")
    ...
    combined.to_csv(output_dir / "welfare_trajectory_with_RR_safety.csv", index=False)
    ...
    fig.tight_layout()
    fig.savefig(figures / "welfare_trajectory_with_RR_safety.png", dpi=220)
    ...
    with (output_dir / "README.md").open("w", encoding="utf-8") as f:
        ...
        f.write("It compares the primitive three-action baseline against a safety-threshold rule applied without the RR mechanism.\n\n")
        ...
        f.write("- `welfare_trajectory_mechanism_ablation.csv` and `figures/welfare_trajectory_mechanism_ablation.png`: combined welfare paths for the no-mechanism primitive baseline, safety-only ablation, RR baseline, and RR plus safety-threshold extension.\n")
        f.write("- `welfare_trajectory_with_RR_safety.csv` and `figures/welfare_trajectory_with_RR_safety.png`: same combined mechanism-ablation output, retained for backward compatibility.\n")
        if main_results_dir is not None:
            f.write(f"\nRR plus safety-threshold data source: `{main_results_dir}`.\n")
        if timing_results_dir is not None:
