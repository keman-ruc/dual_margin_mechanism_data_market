...
def compute_externality_boundary(params: pd.DataFrame, cfg: ExperimentConfig) -> float:
    ...
    reference_exposure = cfg.safety_fee_threshold_quantile * total_d
...
def rr_share_basis_vector(state: np.ndarray, params: pd.DataFrame, cfg: ExperimentConfig) -> np.ndarray:
    if cfg.rr_share_basis != "effective_contribution":
        raise ValueError(
            "RR base shares must use effective_contribution to remain budget balanced"
        )
...
def compute_rr_utility_for_action(
    state: np.ndarray,
    idx: int,
    action: int,
    params: pd.DataFrame,
    ...
    revenue = market_revenue(t_total, cfg.market_revenue_p0, cfg.market_revenue_p1)
    # RR applies only to cooperative participants. Non-participants
    ...
    share_basis = rr_share_basis_vector(cf_state, params, cfg)
...
def compute_utilities_with_rr(
    ...
    for idx in range(len(state)):
        u_n[idx] = compute_rr_utility_for_action(state, idx, STATE_N, params, cfg, beta, core_idx)
        u_i[idx] = compute_rr_utility_for_action(state, idx, STATE_E, params, cfg, beta, core_idx)
        u_f[idx] = compute_rr_utility_for_action(state, idx, STATE_F, params, cfg, beta, core_idx)

    utility_block["U_N_rr"] = u_n
    utility_block["U_E_rr"] = u_i
    utility_block["U_F_rr"] = u_f
    utility_block["beta_rr"] = np.full(len(state), beta, dtype=float)
    utility_block["core_indicator"] = np.isin(np.arange(len(state)), core_idx).astype(int)
    return utility_block


def compute_utilities_with_safety_fee(
    state: np.ndarray,
    params: pd.DataFrame,
    cfg: ExperimentConfig,
    ...
    for idx in range(len(state)):
        ...
        pivot_vec[idx] = pivot
        u_f_dep[idx] = utility_block["U_F"][idx] - pivot * safety_fee_vector[idx]
...
def compute_utilities_with_rr_safety_fee(
    state: np.ndarray,
    params: pd.DataFrame,
    cfg: ExperimentConfig,
    beta: float,
    ...
    utility_block = compute_utilities_with_rr(state, params, cfg, beta=beta)
    current_c_ext = compute_externality_total(state, params, cfg)

    u_f_rr_dep = utility_block["U_F_rr"].copy()
    pivot_vec = np.zeros(len(state), dtype=float)
    for idx in range(len(state)):
        if safety_fee_vector[idx] <= 0.0:
            continue
        ...
        pivot = float(current_c_ext <= c_star < cf_c_ext)
        pivot_vec[idx] = pivot
        u_f_rr_dep[idx] = utility_block["U_F_rr"][idx] - pivot * safety_fee_vector[idx]

    utility_block["U_F_rr_dep"] = u_f_rr_dep
    utility_block["pivot_vector"] = pivot_vec
    utility_block["current_c_ext"] = np.full(len(state), current_c_ext, dtype=float)
    return utility_block
...
def dynamic_safety_fee_vector(
    utility_block: Dict[str, np.ndarray],
    multiple: float = 1.0,
) -> np.ndarray:
    """Compute the current-state minimum fee for the F upgrade margin."""
    gain = utility_block["U_F_rr"] - np.maximum(
        utility_block["U_E_rr"], utility_block["U_N_rr"]
    )
...
def apply_batch_safety_threshold_update(
    ...
    blocked_count = 0
    active_safety_fee = (
        dynamic_safety_fee_vector(utility_block, multiple=safety_fee_multiple)
        if dynamic_safety_fee
        else safety_fee_vector
    )
    if trigger:
        for idx in update_agents:
            if pre_state[idx] != STATE_F or active_safety_fee[idx] <= 0.0:
                continue
            charged_count += 1
            revised_action = choose_action(
                ...
                u_n=float(utility_block["U_N_rr"][idx]),
                u_i=float(utility_block["U_E_rr"][idx]),
                u_f=float(utility_block["U_F_rr"][idx] - active_safety_fee[idx]),
            )
    ...
    return next_state, {
        ...
        "batch_mean_active_safety_fee": float(np.mean(active_safety_fee[update_agents])) if len(update_agents) else 0.0,
    }
