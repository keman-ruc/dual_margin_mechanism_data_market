...
class ExperimentConfig:
    ...
    rhood_feasibility_solo_multiplier: float = 80.0
    rhood_share_basis: str = "effective_contribution"
    beta_grid: Sequence[float] = field(
        default_factory=lambda: tuple(round(x, 2) for x in [i * 0.05 for i in range(21)])
    )

    deposit_noise_sigma: float = 0.05
    deposit_dynamic: bool = True
    deposit_threshold_quantile: float = 0.52
    deposit_multipliers: Sequence[float] = field(
        default_factory=lambda: (0.0, 0.5, 1.0, 1.5, 2.0)
    )
    deposit_candidate_share_grid: Sequence[float] = field(
