...
from sim_runner import run_single_simulation, run_single_simulation_with_deposit, run_single_simulation_with_rhood
from sim_theory import (
    build_candidate_state_from_share,
    compute_entry_margin,
    compute_high_externality_candidate,
    compute_rhood_theory,
    compute_social_benchmark_table,
    compute_upgrade_margin,
    scan_deposit_deterrence,
    scan_rhood_feasibility,
    scan_rhood_theory_curve,
    run_async_theory_with_deposit,
    run_async_theory_with_rhood,
    run_theory_closure_from_seed,
    run_theory_closure_with_deposit,
    run_theory_closure_with_rhood,
)
...
FIGURE_NAMES = {
    ...
    "prop4_beta0": "fig06_proposition_4_beta0_boundary_and_redistribution_feasibility.png",
    "prop4_share": "fig07_proposition_4_final_strategy_shares_theory_vs_simulation.png",
    "prop4_traj": "fig06d_prop4_rhood_strategy_trajectory_theory_vs_simulation.png",
    ...
    "mech_deposit_seed": "fig10c_safety_seed_sensitivity.png",
    "mech_deposit_share": "fig11_main_mechanism_final_strategy_shares.png",
    "mech_deposit_traj": "fig08b_mechanism_strategy_trajectory_theory_vs_simulation.png",
...
def theory_tables_for_run(
    ...
    rhood_state = high_state["state_h"]
    rhood_params = params.copy()
    rhood_params["pi_solo"] = (
        rhood_params["pi_solo"] * cfg.rhood_feasibility_solo_multiplier
    )
    rhood_params["pi_break"] = rhood_params["pi_solo"]
    rhood = compute_rhood_theory(rhood_state, rhood_params, cfg)
    rhood_scan = scan_rhood_feasibility(rhood_state, rhood_params, cfg)
    ...
    if candidate_meta_df.empty:
        candidate_meta_df = pd.DataFrame(
            [
                {
                    "candidate_share_f": float(cfg.deposit_candidate_share_grid[0]),
                    "candidate_c_ext": 0.0,
        ...
        candidate_objects = [
            {
                **candidate_meta_df.iloc[0].to_dict(),
                "state": build_candidate_state_from_share(params, cfg, cfg.deposit_candidate_share_grid[0])["state"],
            }
        ]
    ...
    deposit = scan_deposit_deterrence(best_candidate["state"], params, cfg, c_star=c_star, beta=beta_rhood)
    deposit_agent_df = deposit["agent_df"].copy()
    if not deposit_agent_df.empty:
        deposit_agent_df["candidate_share_f"] = best_candidate["candidate_share_f"]
        deposit_agent_df["candidate_local_eq_rate"] = best_candidate["candidate_local_eq_rate"]
        deposit_agent_df["c_star"] = c_star

    deposit_scan_df = deposit["scan_df"].copy()
    if not deposit_scan_df.empty:
        deposit_scan_df["candidate_share_f"] = best_candidate["candidate_share_f"]
        deposit_scan_df["candidate_local_eq_rate"] = best_candidate["candidate_local_eq_rate"]
        deposit_scan_df["c_star"] = c_star
    ...
    return {
        ...
        "rhood_beta0": rhood["beta0_df"],
        "rhood_scan": rhood_scan,
        "deposit_theory": deposit_agent_df,
        "deposit_scan": deposit_scan_df,
        "candidate_meta": candidate_meta_df,
        "selected_candidate_meta": selected_candidate_meta_df,
    }


def build_deposit_vector_from_candidate(
    cfg: ExperimentConfig,
    params: pd.DataFrame,
    ...
    deposit = scan_deposit_deterrence(candidate_state, params, cfg, c_star=c_star, beta=beta)
    deposit_vector = np.zeros(cfg.n_agents, dtype=float)
    agent_df = deposit["agent_df"].dropna(subset=["D_min"]).copy()
    for row in agent_df.itertuples(index=False):
        deposit_vector[int(row.agent_id)] = float(row.D_min)
    return deposit_vector


def select_rhood_beta(cfg: ExperimentConfig, params: pd.DataFrame) -> float:
    high_state = compute_high_externality_candidate(params, cfg)
    rhood_scan = scan_rhood_feasibility(high_state["state_h"], params, cfg)
    feasible_beta = rhood_scan[rhood_scan["all_core_feasible"] >= 1].sort_values("beta")
    if not feasible_beta.empty:
        return float(feasible_beta.iloc[0]["beta"])
    return float(rhood_scan.sort_values("beta").iloc[0]["beta"])
...
def monte_carlo_experiment(cfg: ExperimentConfig) -> Dict[str, pd.DataFrame]:
    ...
    rhood_seed_rows: List[Dict[str, object]] = []
    rhood_seed_theory_rows: List[Dict[str, object]] = []
    deposit_seed_rows: List[Dict[str, object]] = []
    deposit_seed_theory_rows: List[Dict[str, object]] = []
    strategy_share_rows: List[Dict[str, object]] = []
    strategy_trajectory_rows: List[Dict[str, object]] = []
    rhood_strategy_share_rows: List[Dict[str, object]] = []
    rhood_strategy_trajectory_rows: List[Dict[str, object]] = []
    deposit_strategy_share_rows: List[Dict[str, object]] = []
    deposit_strategy_trajectory_rows: List[Dict[str, object]] = []
    welfare_trajectory_rows: List[Dict[str, object]] = []
    welfare_subset_rows: List[Dict[str, object]] = []
    rhood_beta_rows: List[Dict[str, object]] = []
    rhood_scan_rows: List[Dict[str, object]] = []
    rhood_theory_curve_rows: List[Dict[str, object]] = []
    deposit_rows: List[Dict[str, object]] = []
    deposit_scan_rows: List[Dict[str, object]] = []
    ...
    for run_id in range(cfg.n_runs):
        ...
        deposit_vector = build_deposit_vector_from_candidate(
            cfg,
            params,
            candidate_state,
            c_star=c_star,
        ...
        deposit_result = run_single_simulation_with_deposit(
            cfg,
            params,
            seed_share_f=seed_share,
            beta=beta_star,
        ...
        dep_summary = deposit_result["summary"]
        deposit_strategy_share_rows.append(
            {
                "run_id": run_id,
                "source": "simulation_deposit",
        ...
        dep_traj = deposit_result["trajectory"][["t", "share_N", "share_E", "share_F"]].copy()
        dep_traj["run_id"] = run_id
        dep_traj["source"] = "simulation_deposit"
        deposit_strategy_trajectory_rows.extend(dep_traj.to_dict(orient="records"))
        dep_welfare_traj = deposit_result["trajectory"][["t", "W"]].copy()
        dep_welfare_traj["run_id"] = run_id
        dep_welfare_traj["source"] = "simulation_deposit"
        dep_welfare_traj["mechanism"] = "deposit"
        welfare_trajectory_rows.extend(dep_welfare_traj.to_dict(orient="records"))
        dep_local_welfare = deposit_result.get("local_welfare", pd.DataFrame())
        if not dep_local_welfare.empty:
            dep_local_welfare = dep_local_welfare.copy()
            dep_local_welfare["run_id"] = run_id
            dep_local_welfare["source"] = "simulation_deposit"
            dep_local_welfare["mechanism"] = "deposit"
            welfare_subset_rows.extend(dep_local_welfare.to_dict(orient="records"))

        deposit_theory = run_theory_closure_with_deposit(
            cfg,
            params,
            seed_share_f=seed_share,
            beta=beta_star,
        ...
        dep_theory_state = deposit_theory["final_state"]
        deposit_strategy_share_rows.append(
            {
                "run_id": run_id,
                "source": "theory_deposit",
        ...
        dep_theory_traj = deposit_theory["trajectory"][["t", "share_N", "share_E", "share_F"]].copy()
        dep_theory_traj["run_id"] = run_id
        dep_theory_traj["source"] = "theory_deposit"
        deposit_strategy_trajectory_rows.extend(dep_theory_traj.to_dict(orient="records"))
        dep_theory_welfare_traj = deposit_theory["trajectory"][["t", "W"]].copy()
        dep_theory_welfare_traj["run_id"] = run_id
        dep_theory_welfare_traj["source"] = "theory_deposit"
        dep_theory_welfare_traj["mechanism"] = "deposit"
        welfare_trajectory_rows.extend(dep_theory_welfare_traj.to_dict(orient="records"))

        dep_async_theory = run_async_theory_with_deposit(
            cfg,
            params,
            beta=beta_star,
            deposit_vector=deposit_vector,
        ...
        dep_async_theory_traj["source"] = "theory_deposit_async"
        deposit_strategy_trajectory_rows.extend(dep_async_theory_traj.to_dict(orient="records"))
        dep_async_theory_welfare_traj = dep_async_theory["trajectory"][["t", "W"]].copy()
        dep_async_theory_welfare_traj["run_id"] = run_id
        dep_async_theory_welfare_traj["source"] = "theory_deposit_async"
        dep_async_theory_welfare_traj["mechanism"] = "deposit"
        welfare_trajectory_rows.extend(dep_async_theory_welfare_traj.to_dict(orient="records"))

        rhood_tmp = theory["rhood_beta0"].copy()
        if not rhood_tmp.empty:
            rhood_tmp["run_id"] = run_id
            rhood_beta_rows.extend(rhood_tmp.to_dict(orient="records"))
            theory_curve_tmp = scan_rhood_theory_curve(theory["rhood_beta0"], cfg)
            if not theory_curve_tmp.empty:
                theory_curve_tmp["run_id"] = run_id
                rhood_theory_curve_rows.extend(theory_curve_tmp.to_dict(orient="records"))

        rhood_scan_tmp = theory["rhood_scan"].copy()
        if not rhood_scan_tmp.empty:
            rhood_scan_tmp["run_id"] = run_id
            rhood_scan_rows.extend(rhood_scan_tmp.to_dict(orient="records"))
            rhood_result = run_single_simulation_with_rhood(
                cfg,
                params,
                seed_share_f=seed_share,
                beta=beta_star,
            ...
            rho_summary = rhood_result["summary"]
            rhood_strategy_share_rows.append(
                {
                    "run_id": run_id,
                    "source": "simulation_rhood",
            ...
            rho_traj = rhood_result["trajectory"][["t", "share_N", "share_E", "share_F"]].copy()
            rho_traj["run_id"] = run_id
            rho_traj["source"] = "simulation_rhood"
            rho_traj["beta_rhood"] = beta_star
            rhood_strategy_trajectory_rows.extend(rho_traj.to_dict(orient="records"))
            rho_welfare_traj = rhood_result["trajectory"][["t", "W"]].copy()
            rho_welfare_traj["run_id"] = run_id
            rho_welfare_traj["source"] = "simulation_rhood"
            rho_welfare_traj["beta_rhood"] = beta_star
            rho_welfare_traj["mechanism"] = "rhood"
            welfare_trajectory_rows.extend(rho_welfare_traj.to_dict(orient="records"))
            rho_local_welfare = rhood_result.get("local_welfare", pd.DataFrame())
            if not rho_local_welfare.empty:
                rho_local_welfare = rho_local_welfare.copy()
                rho_local_welfare["run_id"] = run_id
                rho_local_welfare["source"] = "simulation_rhood"
                rho_local_welfare["beta_rhood"] = beta_star
                rho_local_welfare["mechanism"] = "rhood"
                welfare_subset_rows.extend(rho_local_welfare.to_dict(orient="records"))

            rhood_theory = run_theory_closure_with_rhood(
                cfg,
                params,
                seed_share_f=seed_share,
                beta=beta_star,
            ...
            rho_theory_state = rhood_theory["final_state"]
            rhood_strategy_share_rows.append(
                {
                    "run_id": run_id,
                    "source": "theory_rhood",
            ...
            rho_theory_traj = rhood_theory["trajectory"][["t", "share_N", "share_E", "share_F"]].copy()
            rho_theory_traj["run_id"] = run_id
            rho_theory_traj["source"] = "theory_rhood"
            rho_theory_traj["beta_rhood"] = beta_star
            rhood_strategy_trajectory_rows.extend(rho_theory_traj.to_dict(orient="records"))
            rho_theory_welfare_traj = rhood_theory["trajectory"][["t", "W"]].copy()
            rho_theory_welfare_traj["run_id"] = run_id
            rho_theory_welfare_traj["source"] = "theory_rhood"
            rho_theory_welfare_traj["beta_rhood"] = beta_star
            rho_theory_welfare_traj["mechanism"] = "rhood"
            welfare_trajectory_rows.extend(rho_theory_welfare_traj.to_dict(orient="records"))

            rhood_async_theory = run_async_theory_with_rhood(
                cfg,
                params,
                beta=beta_star,
                initial_state=shared_initial_state,
            ...
            rho_async_theory_state = rhood_async_theory["final_state"]
            rhood_strategy_share_rows.append(
                {
                    "run_id": run_id,
                    "source": "theory_rhood_async",
            ...
            rho_async_theory_traj = rhood_async_theory["trajectory"][["t", "share_N", "share_E", "share_F"]].copy()
            rho_async_theory_traj["run_id"] = run_id
            rho_async_theory_traj["source"] = "theory_rhood_async"
            rho_async_theory_traj["beta_rhood"] = beta_star
            rhood_strategy_trajectory_rows.extend(rho_async_theory_traj.to_dict(orient="records"))
            rho_async_theory_welfare_traj = rhood_async_theory["trajectory"][["t", "W"]].copy()
            rho_async_theory_welfare_traj["run_id"] = run_id
            rho_async_theory_welfare_traj["source"] = "theory_rhood_async"
            rho_async_theory_welfare_traj["beta_rhood"] = beta_star
            rho_async_theory_welfare_traj["mechanism"] = "rhood"
            welfare_trajectory_rows.extend(rho_async_theory_welfare_traj.to_dict(orient="records"))

        dep_tmp = theory["deposit_theory"].copy()
        if not dep_tmp.empty:
            dep_tmp["run_id"] = run_id
            deposit_rows.extend(dep_tmp.to_dict(orient="records"))

        dep_scan_tmp = theory["deposit_scan"].copy()
        if not dep_scan_tmp.empty:
            dep_scan_tmp["run_id"] = run_id
            deposit_scan_rows.extend(dep_scan_tmp.to_dict(orient="records"))
    ...
    for seed_share in cfg.seed_scan_grid:
        ...
        rhood_success_flags = []
        rhood_mean_final_share_f = []
        rhood_theory_high_flags = []
        rhood_theory_final_share_f = []
        deposit_success_flags = []
        deposit_mean_final_share_f = []
        deposit_theory_high_flags = []
        deposit_theory_final_share_f = []
        for rep in range(max(8, cfg.n_runs // 3)):
            ...
            high_state = compute_high_externality_candidate(params, cfg)
            rhood_scan_tmp = scan_rhood_feasibility(high_state["state_h"], params, cfg)
            beta_star = select_rhood_beta(cfg, params)
            ...
            rhood_success_flags.append(int(rhood_result["summary"]["steady_state_label"] == "high_externality"))
            rhood_mean_final_share_f.append(float(rhood_result["summary"]["final_share_F"]))
            rhood_theory_result = run_theory_closure_with_rhood(
                cfg,
                params,
                seed_share_f=seed_share,
                beta=beta_star,
            ...
            rhood_theory_high_flags.append(int(rhood_theory_result["high_externality_indicator"]))
            rhood_theory_final_share_f.append(float(rhood_theory_result["final_share_F"]))
            ...
            if best_candidate is None:
                best_candidate = build_candidate_state_from_share(params, cfg, cfg.deposit_candidate_share_grid[0])
            ...
            deposit_success_flags.append(int(deposit_result["summary"]["steady_state_label"] == "high_externality"))
            deposit_mean_final_share_f.append(float(deposit_result["summary"]["final_share_F"]))
            deposit_theory_result = run_theory_closure_with_deposit(
                cfg,
                params,
                seed_share_f=seed_share,
                beta=beta_star,
            ...
            deposit_theory_high_flags.append(int(deposit_theory_result["high_externality_indicator"]))
            deposit_theory_final_share_f.append(float(deposit_theory_result["final_share_F"]))
        seed_rows.append(
            {
                "seed_share_f": seed_share,
        ...
        rhood_seed_rows.append(
            {
                "seed_share_f": seed_share,
                "high_externality_probability": float(np.mean(rhood_success_flags)),
                "mean_final_share_F": float(np.mean(rhood_mean_final_share_f)),
            }
        )
        rhood_seed_theory_rows.append(
            {
                "seed_share_f": seed_share,
                "theory_high_externality_rate": float(np.mean(rhood_theory_high_flags)),
                "theory_mean_final_share_F": float(np.mean(rhood_theory_final_share_f)),
            }
        )
        deposit_seed_rows.append(
            {
                "seed_share_f": seed_share,
                "high_externality_probability": float(np.mean(deposit_success_flags)),
                "mean_final_share_F": float(np.mean(deposit_mean_final_share_f)),
            }
        )
        deposit_seed_theory_rows.append(
            {
                "seed_share_f": seed_share,
                "theory_high_externality_rate": float(np.mean(deposit_theory_high_flags)),
                "theory_mean_final_share_F": float(np.mean(deposit_theory_final_share_f)),
            }
        )
    ...
    mechanism_specs = [
        ...
        ("rhood", "simulation_rhood", "theory_rhood_async", rhood_strategy_share_rows),
        ("safety_threshold_dm", "simulation_deposit", "theory_deposit_async", deposit_strategy_share_rows),
    ]
    ...
    return {
        ...
        "rhood_seed_scan": pd.DataFrame(rhood_seed_rows),
        "rhood_seed_theory_scan": pd.DataFrame(rhood_seed_theory_rows),
        "deposit_seed_scan": pd.DataFrame(deposit_seed_rows),
        "deposit_seed_theory_scan": pd.DataFrame(deposit_seed_theory_rows),
        ...
        "rhood_strategy_share_comparison": pd.DataFrame(rhood_strategy_share_rows),
        "rhood_strategy_trajectory_comparison": pd.DataFrame(rhood_strategy_trajectory_rows),
        "deposit_strategy_share_comparison": pd.DataFrame(deposit_strategy_share_rows),
        "deposit_strategy_trajectory_comparison": pd.DataFrame(deposit_strategy_trajectory_rows),
        ...
        "rhood_beta0": pd.DataFrame(rhood_beta_rows),
        "rhood_scan": pd.DataFrame(rhood_scan_rows),
        "rhood_theory_curve": pd.DataFrame(rhood_theory_curve_rows),
        "deposit_theory": pd.DataFrame(deposit_rows),
        "deposit_scan": pd.DataFrame(deposit_scan_rows),
...
def plot_outputs(cfg: ExperimentConfig, data: Dict[str, pd.DataFrame], output_dir: Path) -> None:
    ...
    rhood_seed_df = data["rhood_seed_scan"]
    rhood_seed_theory_df = data["rhood_seed_theory_scan"]
    deposit_seed_df = data["deposit_seed_scan"]
    deposit_seed_theory_df = data["deposit_seed_theory_scan"]
    ...
    rhood_strategy_share_df = data["rhood_strategy_share_comparison"]
    rhood_strategy_trajectory_df = data["rhood_strategy_trajectory_comparison"]
    deposit_strategy_share_df = data["deposit_strategy_share_comparison"]
    deposit_strategy_trajectory_df = data["deposit_strategy_trajectory_comparison"]
    ...
    rhood_df = data["rhood_beta0"]
    rhood_scan_df = data["rhood_scan"]
    rhood_theory_curve_df = data["rhood_theory_curve"]
    deposit_scan_df = data["deposit_scan"]
    ...
    fig, ax = plt.subplots(figsize=(8, 5))
    ax.step(
        deposit_seed_df["seed_share_f"],
        deposit_seed_df["high_externality_probability"],
        where="post",
        linewidth=2.2,
        label="Simulated probability",
    )
    if not deposit_seed_theory_df.empty:
        ax.step(
            deposit_seed_theory_df["seed_share_f"],
            deposit_seed_theory_df["theory_high_externality_rate"],
            where="post",
            linewidth=2.0,
            color="#C44E52",
    ...
    ax.grid(alpha=0.3)
    fig.tight_layout()
    fig.savefig(figures / FIGURE_NAMES["mech_deposit_seed"], dpi=220)
    plt.close(fig)

    fig, ax = plt.subplots(figsize=(8, 5))
    ax.plot(
        rhood_seed_df["seed_share_f"],
        rhood_seed_df["high_externality_probability"],
        marker="o",
        linewidth=2.2,
        label="RR",
    )
    ...
    if rhood_scan_df.empty:
        ax1.text(0.5, 0.5, "No redistribution scan results observed", ha="center", va="center", transform=ax1.transAxes)
        ax1.set_axis_off()
    else:
        grouped_scan = rhood_scan_df.groupby("beta", as_index=False)["all_core_feasible"].mean()
        ...
        if not rhood_theory_curve_df.empty:
            grouped_theory_curve = rhood_theory_curve_df.groupby("beta", as_index=False)["all_core_feasible_theory"].mean()
            ax1.plot(
        ...
        ax1.set_xlabel("Redistribution rate beta")
        ...
        ax1.set_title("Proposition 4: stressed beta_0 vs redistribution feasibility")
    fig.tight_layout()
    fig.savefig(figures / FIGURE_NAMES["prop4_beta0"], dpi=220)
    plt.close(fig)

    fig, ax = plt.subplots(figsize=(8, 5))
    if deposit_scan_df.empty:
        ax.text(0.5, 0.5, "No positive-pivot agents in candidate safe states", ha="center", va="center", transform=ax.transAxes)
        ax.set_axis_off()
    else:
        grouped_dep = deposit_scan_df.groupby("deposit_multiple", as_index=False)["deterrence_success_rate"].mean()
        ax.step(
            grouped_dep["deposit_multiple"],
    ...
    fig, ax = plt.subplots(figsize=(8, 5))
    if rhood_strategy_share_df.empty:
        ax.text(0.5, 0.5, "No redistribution strategy share comparison available", ha="center", va="center", transform=ax.transAxes)
        ax.set_axis_off()
    else:
        grouped = rhood_strategy_share_df.groupby("source", as_index=False)[["share_N", "share_E", "share_F"]].mean()
        subset = grouped.set_index("source").reindex(["theory_rhood_async", "simulation_rhood"]).fillna(0.0)
        ...
        ax.set_title("Redistribution-regime final strategy shares")
    ...
    fig, axes = plt.subplots(3, 1, figsize=(8, 10), sharex=True)
    if rhood_strategy_trajectory_df.empty:
        for ax in axes:
            ax.text(0.5, 0.5, "No redistribution trajectory comparison available", ha="center", va="center", transform=ax.transAxes)
            ax.set_axis_off()
    else:
        rhood_plot_df = padded_strategy_for_plot(rhood_strategy_trajectory_df, ["simulation_rhood"])
        grouped_traj = (
            rhood_plot_df.groupby(["source", "t"], as_index=False)[["share_N", "share_E", "share_F"]]
            .mean()
        )
        styles = {
            "theory_rhood_async": ("--", "#C44E52", "Async theory"),
            "simulation_rhood": ("-", "#4C72B0", "Simulation mean"),
        }
        series_info = [("share_N", "N share"), ("share_E", "E share"), ("share_F", "F share")]
        for ax, (col, title) in zip(axes, series_info):
            for source, (linestyle, color, label) in styles.items():
            ...
            ax.set_ylim(0.0, 1.0)
            ax.grid(alpha=0.3)
        axes[0].set_title("Redistribution N / E / F trajectory: theory vs simulation")
        axes[-1].set_xlabel("Round t")
        axes[0].legend(loc="best")
    fig.tight_layout()
    ...
    fig, ax = plt.subplots(figsize=(8, 5))
    if deposit_strategy_share_df.empty:
        ax.text(0.5, 0.5, "No safety-threshold strategy share comparison available", ha="center", va="center", transform=ax.transAxes)
        ax.set_axis_off()
    else:
        grouped = deposit_strategy_share_df.groupby("source", as_index=False)[["share_E", "share_F"]].mean()
        sim_row = grouped[grouped["source"] == "simulation_deposit"]
        theory_row = grouped[grouped["source"] == "theory_deposit_async"]
        labels = ["E", "F"]
        x = np.arange(len(labels))
        width = 0.34
        ...
        ax.grid(axis="y", alpha=0.3)
    fig.tight_layout()
    fig.savefig(figures / FIGURE_NAMES["mech_deposit_share"], dpi=220)
    plt.close(fig)

    fig, axes = plt.subplots(3, 1, figsize=(8, 10), sharex=True)
    if deposit_strategy_trajectory_df.empty:
        for ax in axes:
            ax.text(0.5, 0.5, "No safety-threshold trajectory comparison available", ha="center", va="center", transform=ax.transAxes)
            ax.set_axis_off()
    else:
        deposit_plot_df = padded_strategy_for_plot(deposit_strategy_trajectory_df, ["simulation_deposit"])
        grouped_traj = (
            deposit_plot_df.groupby(["source", "t"], as_index=False)[["share_N", "share_E", "share_F"]]
            .mean()
        )
        styles = {
            "theory_deposit_async": ("--", "#C44E52", "Async theory"),
            "simulation_deposit": ("-", "#4C72B0", "Simulation mean"),
        }
        series_info = [("share_N", "N share"), ("share_E", "E share"), ("share_F", "F share")]
        for ax, (col, title) in zip(axes, series_info):
            for source, (linestyle, color, label) in styles.items():
        ...
        axes[0].legend(loc="best")
    fig.tight_layout()
    fig.savefig(figures / FIGURE_NAMES["mech_deposit_traj"], dpi=220)
    plt.close(fig)
    ...
    if three_mechanisms_share_df.empty:
        ...
        mechanisms = ["rhood_theory", "rhood_sim", "deposit_theory", "deposit_sim"]
        labels = ["Redistribution\ntheory", "Redistribution\nsim", "Safety\ntheory", "Safety\nsim"]
        colors = {"share_N": "#4C72B0", "share_E": "#55A868", "share_F": "#C44E52"}
        rhood_grouped = (
            rhood_strategy_share_df.groupby("source", as_index=False)[["share_N", "share_E", "share_F"]]
            .mean()
            .set_index("source")
        )
        deposit_grouped = (
            deposit_strategy_share_df.groupby("source", as_index=False)[["share_N", "share_E", "share_F"]]
            .mean()
            .set_index("source")
        )
        subset = pd.DataFrame(
            [
                rhood_grouped.reindex(["theory_rhood_async"]).fillna(0.0).iloc[0],
                rhood_grouped.reindex(["simulation_rhood"]).fillna(0.0).iloc[0],
                deposit_grouped.reindex(["theory_deposit_async"]).fillna(0.0).iloc[0],
                deposit_grouped.reindex(["simulation_deposit"]).fillna(0.0).iloc[0],
            ],
            index=mechanisms,
        )
    ...
    if welfare_trajectory_df.empty:
        ...
        plot_sources = ["simulation_rhood", "theory_rhood_async", "simulation_deposit", "theory_deposit_async"]
        padded_sim = padded_welfare_for_plot(welfare_trajectory_df, ["simulation_rhood", "simulation_deposit"])
        theory_part = welfare_trajectory_df[welfare_trajectory_df["source"].isin(["theory_rhood_async", "theory_deposit_async"])]
        ...
        styles = {
            "simulation_rhood": ("-", "#55A868", "Redistribution sim"),
            "theory_rhood_async": ("--", "#8172B3", "Redistribution async theory"),
            "simulation_deposit": ("-", "#DD8452", "Safety-threshold sim"),
            "theory_deposit_async": ("--", "#8C8C8C", "Safety-threshold async theory"),
        }
    ...
    if welfare_subset_df.empty:
        ...
        styles = {
            "simulation_rhood": ("#4C72B0", "Redistribution subset"),
            "simulation_deposit": ("#DD8452", "Safety-threshold subset"),
        }
    ...
    if rhood_scan_df.empty:
        ...
        ax.set_xlabel("Redistribution rate beta")
        ax.set_ylabel("Run-level feasibility rate")
        ax.set_ylim(-0.05, 1.05)
        ax.set_title("Prop. 4: redistribution feasibility boundary")
        ax.grid(alpha=0.3)
        ax.legend()
...
def load_existing_outputs(output_dir: Path) -> Dict[str, pd.DataFrame]:
    ...
    required = {
        ...
        "seed_theory_scan",
        "rhood_seed_scan",
        "rhood_seed_theory_scan",
        "deposit_seed_scan",
        "deposit_seed_theory_scan",
        "strategy_share_comparison",
        "strategy_trajectory_comparison",
        "rhood_strategy_share_comparison",
        "rhood_strategy_trajectory_comparison",
        "deposit_strategy_share_comparison",
        "deposit_strategy_trajectory_comparison",
